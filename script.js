let items = [
    ...
    function removeItem(i) {
      items.splice(i, 1); renderEditors(); renderInvoice();
    }
    function renderEditors() {
      ...
      <div class="item-editor">
        <button class="danger remove" onclick="removeItem(${i})">Hapus</button>
    ...
    function renderInvoice() {
      ...
      body.innerHTML = '';
      items.forEach(it => {
        const total = (Number(it.qty) || 0) * (Number(it.price) || 0); subtotal += total;
    ...
    function saveData() {
      ...
      alert('Draft invoice tersimpan di browser.');
    }
    function loadData() {
      const raw = localStorage.getItem('pulbaInvoiceDraft');
      if (!raw) { alert('Belum ada draft tersimpan.'); return }
